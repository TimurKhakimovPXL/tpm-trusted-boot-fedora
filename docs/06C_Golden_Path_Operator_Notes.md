...
Operator guidance for the trusted boot lab rebuild, including the build
sequence and the context needed to understand each part of
`06B_Golden_Path_Rebuild_Runbook.md` without repeating the architecture
chapters.
...
This document provides context and links to the authoritative material. It does
not copy commands from `06B`, current values from `00_state`, design rationale
from `05`, or failure records from `06F`.

---

## 2. The build summary: phases and dependencies
...
3. **Block B.2: kernel-signing automation** (`06B` Steps 27–37). Wraps the manual kernel/UKI rebuild in a DNF post-transaction chain (decider + helper + action rule). If boot inputs changed, it rebuilds, re-signs, re-seals and re-predicts. If convergence fails, DNF reports the post-transaction error and the unsafe-reboot sentinel remains. The already committed RPM transaction is not rolled back, and the sentinel does not technically prevent a reboot. Depends on the hook (Block A) being the trusted signing authority.
...
5. **Module 4: governance, recovery, decommissioning** (architecture and recovery guidance in `04`). Covers key rotation, compromise response, disaster recovery, audit and separation of duties. Site-specific procedures still require local key-custody and platform details.
...
- **PCR 11: UKI / kernel-phase measurements (signed-policy binding).** Records the measured sections of the booted UKI. It changes when a kernel, initramfs, embedded command line or another measured input changes. Rebuilding identical measured content can change the UKI file hash without changing PCR 11.

"Forward sealing" is the trick that makes PCR 11 updatable: instead of sealing the disk key to a *value*, you **predict** what PCR 11 *will be* for the new UKI (with `systemd-measure`, which lives at `/usr/lib/systemd/systemd-measure` and is not on `PATH`), sign that prediction with the policy key, and embed the signed prediction in the UKI's `.pcrsig` section. At boot, `systemd-stub` presents that signed policy to the TPM, which releases the key if the runtime PCR 11 matches. So a new, correctly-built UKI unlocks the disk *before it has ever booted*, with no re-enrollment. Note the consequence operators trip over: a rebuilt UKI's *file* hash changes (new signature/timestamp) while its *predicted PCR 11* stays identical, because PCR 11 measures section content, not the whole-file signature (`06F` E.3 / O.2). See `01` §4–§5, `02`.

### 3.5 The policy key, TPM sealing, and key release

Forward sealing needs a signing key (the "policy keypair"). Its public half is enrolled into the TPM policy at disk-enrollment time (the "welding" in `03` §1); its private half signs each PCR-11 prediction. That private key is itself **sealed into the TPM** so it exists in plaintext only transiently at signing time, then is shredded (`05` §2.2). Compromise of the policy private key is a governance event with a defined response (`04` §4.3). See `01` §5 (policy-based key release), `05` §2.

### 3.6 LUKS2 unlock with the split policy

The encrypted root uses a LUKS2 header with two separate concepts: keyslots and
the TPM policy. A keyslot holds an encrypted copy of the volume key. The TPM
policy defines the conditions under which the TPM-backed slot can be opened.
This lab binds PCR 7 statically and authorises PCR 11 with a signed policy.
Keyslot 0 remains available as the passphrase recovery anchor. A passphrase
prompt after Module 3 indicates that automatic TPM unlock failed and must be
investigated. See `03` for the enrollment details.

### 3.7 The B.2 kernel-signing automation chain (decider + helper + rule)

Manual rebuilds do not cover routine `dnf` use, so B.2 automates them with a
post-transaction action. The rule (`50-tboot-posttrans.actions`) has an empty
`package_filter` and calls `tboot-dnf-posttrans` after every transaction. The
decider runs as root, computes the boot-input manifest and invokes
`tboot-dnf-helper` only when it finds drift. The helper performs the initramfs
and UKI work. After the helper succeeds, the decider verifies the result,
advances the baseline and clears `UNSAFE-TO-REBOOT`. This is a separation of
responsibilities, not a privilege boundary. Failure to create the sentinel
stops the helper before any boot mutation. See `05` §3.3 for the manifest and
baseline policy. The byte-pinned sources are included in `06B` Steps 29, 31 and
33.
...
The install-vs-arm split and the two-chain co-fire that this produces are explained operationally in §4.6 below. See `05` §3.5.

### 3.9 Governance, recovery, decommissioning (Module 4)

Module 4 describes controlled policy-key rotation, compromise response, TPM or
motherboard replacement, separation of duties, audit logging and
decommissioning. The dual-slot rotation still requires a real boot validation
with the recovery passphrase available; it is not a pre-validated zero-downtime
transition. Site-specific recovery details remain the operator's
responsibility.
